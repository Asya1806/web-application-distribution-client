...
import { editSpecialty } from "../../../redux/api/specialty";
...
const EditSpecialty = (props) => {
  ...
  const onClickButtonEditSpec = async (e) => {
    e.preventDefault();
    setModalSpec(false);
  ...
  const toggle = () => setModalSpec((modalSpec) => !modalSpec);
  ...
  return (
    <Fragment>
      {button}
      <Modal isOpen={modalSpec} toggle={toggle}>
        <ModalHeader toggle={toggle}>{titleSpec}</ModalHeader>
        <ModalBody>
          <Form>
            <FormGroup>
            ...
            <FormGroup style={{ textAlign: "center" }}>
              <Button color="success" onClick={onClickButtonEditSpec}>
                Подтвердить
              </Button>
            </FormGroup>
          </Form>
